...
run(['add', 'app/(dashboard)/drivers/page.tsx']);
run(['commit', '-m', 'fix: select all supplier fields in drivers page to satisfy Supplier[] type']);
run(['push', 'origin', 'master']);
run(['log', '--oneline', '-4']);

fs.writeFileSync(cwd + '\\git-push3-log.txt', log.join('\n'));
