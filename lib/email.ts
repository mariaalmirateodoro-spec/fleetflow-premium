...
// Light-background / dark-text palette on purpose — dark-themed HTML emails
// are unreliable across mail clients' auto dark-mode color inversion (Apple
// Mail, Gmail app, etc. often ignore embedded <style> blocks and re-invert
// text color while leaving backgrounds alone, producing unreadable dark-on-
// dark text). A light background is readable everywhere regardless of the
// recipient's dark-mode setting. The purple/indigo brand gradient is kept
// only in the header banner and buttons, where it's decorative, not load-
// bearing for text contrast.
const baseStyles = `
  body { margin: 0; padding: 0; background-color: #f1f2f6; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; }
  .wrapper { max-width: 600px; margin: 0 auto; padding: 40px 20px; }
  .card { background: #ffffff; border: 1px solid rgba(79,70,229,0.15); border-radius: 16px; overflow: hidden; }
  ...
  .greeting { color: #1e293b; font-size: 16px; margin: 0 0 24px; }
  .ref-box { background: rgba(79,70,229,0.06); border: 1px solid rgba(79,70,229,0.25); border-radius: 10px; padding: 16px 20px; text-align: center; margin-bottom: 28px; }
  .ref-label { color: #64748b; font-size: 11px; text-transform: uppercase; letter-spacing: 1px; margin: 0 0 6px; }
  .ref-number { color: #4f46e5; font-size: 26px; font-weight: 700; letter-spacing: 2px; margin: 0; }
  .section-title { color: #64748b; font-size: 11px; text-transform: uppercase; letter-spacing: 1px; margin: 0 0 12px; padding-bottom: 8px; border-bottom: 1px solid rgba(0,0,0,0.06); }
  ...
  .detail-table td:last-child { color: #1e293b; font-size: 13px; font-weight: 500; }
  .status-badge { display: inline-block; padding: 6px 16px; border-radius: 20px; font-size: 13px; font-weight: 600; }
  .badge-approved { background: rgba(16,185,129,0.12); color: #059669; border: 1px solid rgba(16,185,129,0.3); }
  ...
  .note-box { background: rgba(79,70,229,0.05); border-left: 3px solid #4f46e5; border-radius: 0 8px 8px 0; padding: 14px 18px; margin-bottom: 28px; color: #475569; font-size: 13px; line-height: 1.6; }
  ...
  .footer { padding: 24px 40px; border-top: 1px solid rgba(0,0,0,0.06); text-align: center; }
...
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light only"><meta name="supported-color-schemes" content="light only"><title>Booking Received</title>
...
<div class="wrapper">
  <div class="card">
    ...
    <div class="body">
      <p class="greeting">Hi <strong style="color:#1e293b">${data.guestName}</strong>, your transport request has been received!</p>
      ...
      <div class="note-box">
        <strong style="color:#4f46e5">What happens next?</strong><br/>
      ...
      <p style="color:#64748b;font-size:13px;text-align:center;margin:0">
        Questions? Contact our team and quote reference <strong style="color:#4f46e5">${data.referenceNumber}</strong>.
      </p>
    </div>
    <div class="footer">
...
export async function sendBookingApprovedEmail(data: BookingStatusEmailData) {
  ...
  const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light only"><meta name="supported-color-schemes" content="light only"><title>Booking Approved</title>
<style>${baseStyles}</style></head>
<body>
<div class="wrapper">
  <div class="card">
    ...
    <div class="body">
      <p class="greeting">Great news, <strong style="color:#1e293b">${data.guestName}</strong>!</p>
      ...
      <div class="note-box">
        <strong style="color:#4f46e5">Your vehicle will be ready for you.</strong><br/>
      ...
      <p style="color:#64748b;font-size:13px;text-align:center;margin:0">
        Reference: <strong style="color:#4f46e5">${data.referenceNumber}</strong>
      </p>
    </div>
    <div class="footer">
      <p>© ${new Date().getFullYear()} FleetFlow Premium · Internal System</p>
...
export async function sendBookingRejectedEmail(data: BookingStatusEmailData) {
  const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light only"><meta name="supported-color-schemes" content="light only"><title>Booking Update</title>
<style>${baseStyles}</style></head>
<body>
<div class="wrapper">
  <div class="card">
    ...
    <div class="body">
      <p class="greeting">Hi <strong style="color:#1e293b">${data.guestName}</strong>,</p>
      ...
      <p style="color:#64748b;font-size:14px;line-height:1.6;margin:0 0 24px;">
        Unfortunately, we were unable to confirm your transport booking for <strong style="color:#1e293b">${data.pickupLocation} → ${data.dropoffLocation}</strong>.
      </p>
      ...
      <div class="note-box">
        <strong style="color:#4f46e5">Need transport?</strong><br/>
...
export async function sendBookingCancelledEmail(data: BookingCancelledEmailData) {
  ...
  const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light only"><meta name="supported-color-schemes" content="light only"><title>Booking Cancelled</title>
<style>${baseStyles}</style></head>
<body>
<div class="wrapper">
  <div class="card">
    ...
    <div class="body">
      ...
      <p style="color:#64748b;font-size:14px;line-height:1.6;margin:0 0 24px;">
        We regret to inform you that your transport booking from <strong style="color:#1e293b">${data.pickupLocation}</strong> to <strong style="color:#1e293b">${data.dropoffLocation}</strong> scheduled for <strong style="color:#1e293b">${pickup}</strong> has been cancelled.
      </p>
      ...
      <div class="note-box">
        <strong style="color:#4f46e5">Need to rebook or have questions?</strong><br/>
...
export async function sendDriverAssignedEmail(data: DriverAssignedEmailData) {
  ...
  const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light only"><meta name="supported-color-schemes" content="light only"><title>Driver Assigned</title>
<style>${baseStyles}</style></head>
<body>
<div class="wrapper">
  <div class="card">
    ...
    <div class="body">
      ...
      <table class="detail-table">
        <tr><td>Driver Name</td><td><strong style="color:#1e293b">${data.driverName}</strong></td></tr>
        ...
        ${data.vehiclePlate ? `<tr><td>Plate Number</td><td><strong style="color:#4f46e5">${data.vehiclePlate}</strong></td></tr>` : ''}
        <tr><td>Vehicle Type</td><td>${formatVehicleType(data.vehicleType)}</td></tr>
      </table>
      ...
      <div class="note-box">
        <strong style="color:#4f46e5">You're all set!</strong><br/>
...
export async function sendTripReminderEmail(data: TripReminderEmailData) {
  ...
  const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light only"><meta name="supported-color-schemes" content="light only"><title>Your Trip is Tomorrow</title>
<style>${baseStyles}</style></head>
<body>
<div class="wrapper">
  <div class="card">
    ...
    <div class="body">
      <p class="greeting">Hi <strong style="color:#1e293b">${data.guestName}</strong>, your trip is coming up soon!</p>
      ...
      <div style="background:rgba(99,102,241,0.12);border:1px solid rgba(99,102,241,0.35);border-radius:10px;padding:18px 20px;text-align:center;margin-bottom:28px;">
        <p style="color:#64748b;font-size:11px;text-transform:uppercase;letter-spacing:1px;margin:0 0 8px;">Pickup Time</p>
        <p style="color:#4f46e5;font-size:28px;font-weight:700;margin:0 0 4px;">${timeOnly}</p>
      ...
      <p class="section-title">Trip Details</p>
      <table class="detail-table">
        <tr><td>Pickup Location</td><td><strong style="color:#1e293b">${data.pickupLocation}</strong></td></tr>
        <tr><td>Drop-off Location</td><td>${data.dropoffLocation}</td></tr>
        <tr><td>Vehicle Type</td><td>${formatVehicleType(data.vehicleType)}</td></tr>
      </table>
      ...
      <p class="section-title">Your Driver</p>
      <table class="detail-table">
        <tr><td>Driver</td><td><strong style="color:#1e293b">${data.driverName}</strong></td></tr>
      ...
      <div class="note-box">
        <strong style="color:#4f46e5">Friendly reminder:</strong><br/>
        Please be at <strong style="color:#1e293b">${data.pickupLocation}</strong> by <strong style="color:#4f46e5">${timeOnly}</strong> tomorrow. If your plans have changed, please contact our team as soon as possible.
      </div>
...
export async function sendModificationRequestEmail(data: ModificationRequestEmailData) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light only"><meta name="supported-color-schemes" content="light only"><style>${baseStyles}</style></head>
<body>
<div class="wrapper">
  <div class="card">
    <div class="header">
    ...
    <div class="body">
      ...
      <p style="color:#64748b;font-size:13px;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;margin:0 0 12px">Requested Changes</p>
...
// Modification Approved / Rejected guest emails were removed — modification
// requests are still approved/rejected in the dashboard, they just no longer
// send a guest notification email for that step.
...
<div class="wrapper">
  <div class="card">
    ...
    <div class="body">
      <p style="color:#64748b;margin:0 0 20px">Dear <strong style="color:#1e293b">${data.guestName}</strong>, your trip has been completed. Here is your receipt.</p>
      ...
      <div class="divider"></div>
      <p style="color:#64748b;font-size:13px;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;margin:0 0 12px">Trip Details</p>
      ...
      <div class="section" style="background:rgba(99,102,241,0.1);border-radius:8px;padding:16px;margin:0">
        ...
        <p style="font-size:28px;font-weight:700;color:#4f46e5;margin:4px 0 0">$${data.finalCostUsd.toFixed(2)} <span style="font-size:14px;color:#64748b">USD</span></p>
      </div>` : ''}
...
// ─────────────────────────────────────────────────────────────
// Trip Completed — internal staff notification
// ─────────────────────────────────────────────────────────────

interface TripCompletedStaffEmailData {
  staffEmails: string // comma-separated list
  referenceNumber: string
  guestName: string
  pickupLocation: string
  dropoffLocation: string
...
export async function sendTripCompletedStaffEmail(data: TripCompletedStaffEmailData) {
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light only"><meta name="supported-color-schemes" content="light only"><style>${baseStyles}</style></head>
<body>
<div class="wrapper">
  <div class="card">
    ...
    <div class="body">
      <p class="greeting">Booking <strong style="color:#4f46e5">${data.referenceNumber}</strong> has been marked as completed.</p>

      <table class="detail-table">
        <tr><td>Guest</td><td>${data.guestName}</td></tr>
        <tr><td>Pickup Location</td><td>${data.pickupLocation}</td></tr>
        <tr><td>Drop-off Location</td><td>${data.dropoffLocation}</td></tr>
        <tr><td>Vehicle Type</td><td>${formatVehicleType(data.vehicleType)}</td></tr>
        ${data.finalCostUsd != null ? `<tr><td>Final Cost</td><td>$${data.finalCostUsd.toFixed(2)} USD</td></tr>` : ''}
      </table>

      <div class="note-box">This is an automated internal notice — no action needed unless a guest reports an issue.</div>
    </div>
    <div class="footer">
      <p>© ${new Date().getFullYear()} FleetFlow Premium · Internal System</p>
    </div>
  ...
  return await sendEmail(
    data.staffEmails,
    `🏁 Trip Completed — Ref: ${data.referenceNumber}`,
    html,
  )
}

// ─────────────────────────────────────────────────────────────
// Email verification code — sent before a guest booking can be
// finally submitted (draft saves don't require this).
// ─────────────────────────────────────────────────────────────

// ─────────────────────────────────────────────────────────────
// Draft saved — so a guest who closes the tab can still find
// their way back, even if they didn't note down the reference.
// ─────────────────────────────────────────────────────────────

export async function sendDraftSavedEmail(toEmail: string, referenceNumber: string) {
  const siteUrl = process.env.NEXT_PUBLIC_APP_URL?.replace(/\/$/, '') ?? ''
  const resumeUrl = siteUrl ? `${siteUrl}/book/status/${referenceNumber}` : `/book/status/${referenceNumber}`

  const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light only"><meta name="supported-color-schemes" content="light only"><title>Draft Saved</title>
<style>${baseStyles}</style></head>
<body>
<div class="wrapper">
  <div class="card">
    ...
    <div class="body">
      <p class="greeting">Your booking progress has been saved as a draft.</p>

      <div class="ref-box">
        <p class="ref-label">Your Draft Reference</p>
        <p class="ref-number">${referenceNumber}</p>
      </div>

      <a href="${resumeUrl}" class="cta-btn">Continue my booking</a>

      <div class="note-box">
        Nothing has been submitted yet — use the link above (or this reference number) whenever you're ready to finish and submit your request.
      </div>
    </div>
    <div class="footer">
      <p>© ${new Date().getFullYear()} FleetFlow Premium · Internal System</p>
  ...
  return await sendEmail(toEmail, `📝 Your FleetFlow draft — Ref: ${referenceNumber}`, html)
}

export async function sendVerificationCodeEmail(toEmail: string, code: string) {
  const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light only"><meta name="supported-color-schemes" content="light only"><title>Verify your email</title>
<style>${baseStyles}</style></head>
<body>
<div class="wrapper">
  <div class="card">
    ...
    <div class="body">
      <p class="greeting">Please confirm this is really your email address before we submit your booking.</p>

      <div class="ref-box">
        <p class="ref-label">Your verification code</p>
        <p class="ref-number" style="letter-spacing:8px">${code}</p>
      </div>
  ...
  return await sendEmail(toEmail, `Your FleetFlow verification code: ${code}`, html)
}
