...
run(['add', 'components/bookings/BookingDetailModal.tsx']);
run(['commit', '-m', 'feat: open LINE with message pre-filled instead of just copying to clipboard']);
run(['push', 'origin', 'master']);
run(['log', '--oneline', '-4']);

fs.writeFileSync(cwd + '\\git-push4-log.txt', log.join('\n'));
